...
export default function AdminShell({ onLogout }: Props) {
  ...
  return (
    <div className="relative isolate flex h-screen bg-[#F1F4F9]/95 backdrop-blur-[2px] overflow-hidden"><AnimatedBackground variant="subtle" />
      <aside className={`fixed md:relative z-50 w-56 h-full flex-shrink-0 bg-[#07111F] flex flex-col transition-transform duration-200 ${open ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 bg-white border-b border-[#E2E8F0] flex items-center px-4 md:px-5 gap-4">
          <button aria-label="Toggle admin navigation" onClick={() => setOpen(v => !v)} className="md:hidden w-9 h-9 rounded-xl border border-[#E2E8F0] bg-[#F1F4F9] flex items-center justify-center text-[#475569]"><Menu size={18}/></button>
